...
// var EncodingAESKey="1234567890123456789012345678901234567890123"
// var message="中华人民共和国"
// const enstr = AES.encrypt(EncodingAESKey,message)
// console.log( enstr )
// console.log ( AES.decrypt( EncodingAESKey, enstr ))
const ToJson = function (xml) {
  var obj = {}
  var lines = xml.split("\n")
  var obj = {}
  var tmpl = ""
  ...
  for( var i in lines ) { 
    ...
    var sindex = l.indexOf(">")
    var xindex = l.indexOf("<")+1
    var key = l.substr(xindex,sindex-xindex);
    var eindex = l.lastIndexOf("<")
    if( l.substr(sindex+1,9) == "<![CDATA[" ){
      sindex += 9;
      eindex -= 3;
    }
    var val = l.substring(sindex+1,eindex);
    obj[key] = val 
  }
  return obj 
}
...
function handleRequest(r){
    var msg_signature = qs.unescape(r.args['signature']) || ""
    var timestamp = r.args['timestamp'] || ""
    var nonce = qs.unescape(r.args['nonce']) || ""
    var response = ""
    var echostr = qs.unescape(r.args['echostr']) || r.requestBody
    var sign = getSignature(Token,timestamp,nonce)
    ...
    if (r.method.toUpperCase() == "GET" ){        
        ...
        ngx.log(ngx.ERR,r.requestBody )
        var params = ToJson( r.requestBody )
        if( 'Encrypt' in  params ){
            var msgobj= AES.decrypt(EncodingAESKey, params['Encrypt'])
            var posts = ToJson( msgobj['message'] )
            params = Object.assign(params, posts);
        }
        ngx.log(ngx.ERR,JSON.stringify( params ))
        response = "";
    }
